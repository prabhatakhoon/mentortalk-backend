...
async function getServiceAccount() {
  ...
  const response = await secretsClient.send(
    new GetSecretValueCommand({ SecretId: "mentortalk/firebase-service-account" })
  );
...
export async function sendFcmNotification(userId, { title, body, data = {} }, opts = {}) {
  ...
  const result = await db.query(
    `SELECT ${fcmColumn} AS fcm_token FROM "user" WHERE id = $1`,
    [userId]
  );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (error.includes("UNREGISTERED") || error.includes("INVALID_ARGUMENT")) {
        await db.query(
          `UPDATE "user" SET ${fcmColumn} = NULL WHERE id = $1`,
          [userId]
        );
